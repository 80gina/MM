...
3. 새 창에 표시된 `https://...trycloudflare.com` 주소를 복사해 본인 휴대폰의 모바일 데이터로 접속합니다. 주소 끝에 `/api/health`를 붙여 모델 준비 상태도 확인합니다. **각 참가자에게 전달하기 직전에 다시 확인**하세요. 2026-09-19 자체 점검 중 첫 임시 주소는 몇 시간 뒤 만료되어 터널을 재시작하고 새 주소를 받았습니다.
